 ...
 * @author huajie <banhuajie@163.com>
 */
class ZhaopinController extends AdminController {
    ...
    /**
     * 显示左边菜单，进行权限控制
     * @author Gebitily7 <gebitily@yeah.net> 改于20160315
     */
    protected function getMenu($ismenu = null){
        ...
        //生成每个分类的url
        foreach ($cate as $key=>&$value){
            $value['url']   =   'Zhaopin/index?cate_id='.$value['id'];
            ...
            if(!empty($value['_child'])){
                ...
                foreach ($value['_child'] as $ka=>&$va){
                    $va['url']      =   'Zhaopin/index?cate_id='.$va['id'];
                    if(!empty($va['_child'])){
                        foreach ($va['_child'] as $k=>&$v){
                            $v['url']   =   'Zhaopin/index?cate_id='.$v['id'];
                            $v['pid']   =   $va['id'];
    ...
    public function index($cate_id = null, $model_id = null, $position = null,$group_id=null){
        ...
        // 列表查询
        $list   =   $this->getDocumentList($cate_id,$model_id,$position,$fields,$group_id,$this->iscate);
        //var_dump($list);exit;
        // 列表显示处理
     ...
     * @param integer $group_id 分组id
     */
    protected function getDocumentList($cate_id=0,$model_id=null,$position=null,$field=true,$group_id=null,$iscate=null){
        /* 查询条件初始化 */
        $map = array();
        if(isset($_GET['title'])){
    ...
    public function edit(){
        ...
        $data['arr_welfare'] = explode(',',$data['arr_welfare']);
